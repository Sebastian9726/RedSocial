      ...
      <v-layout row wrap>
        <v-flex xs12 sm3 v-for="post in userPosts" :key="post._id">
          <v-card class="mt-3 ml-1 mr-2" hover>
            <v-card-media @click="goToPost(post._id)" height="30vh" :src="post.ruta"></v-card-media>
            <v-card-text>{{ post.nombre }}</v-card-text>
            ...
            <v-layout row>
              <v-flex xs12>
                <v-select
                  ...
                  :items="['Vacaciones ', 'Playa ', 'Bosque', 'Familiar', 'Aniversario', 'Luna de miel', 'Paisaje', 'Relax', 'Extranjero', 'Aventura', 'Tour']"
                  multiple
                  label="Categorías"
                ></v-select>
              </v-flex>
